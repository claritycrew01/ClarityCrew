...
class SessionAnalyzer {
  SessionSummary analyze(SessionRecord session) {
    ...
    final completedCount =
        interactions.where((i) => i.interactionType == 'completed').length;
    final struggledCount =
        interactions.where((i) => i.interactionType == 'struggled').length;
    final skippedCount =
        interactions.where((i) => i.interactionType == 'skipped').length;
    final hintCount =
        interactions.where((i) => i.interactionType == 'tappedHint').length;

    final totalScore = interactions
        .where((i) => i.score != null)
        .fold<int>(0, (sum, i) => sum + (i.score ?? 0));
    final scoredCount = interactions.where((i) => i.score != null).length;
    ...
    final averageDuration = interactions.isEmpty
        ? 0.0
        : interactions.fold<double>(0.0, (sum, i) => sum + i.durationSeconds) /
            interactions.length;
    ...
    return SessionSummary(
      ...
      completionRate:
          interactions.isEmpty ? 0.0 : completedCount / interactions.length,
    );
  }
  ...
  List<String> getWeakAreas(SessionRecord session) {
    ...
    for (final entry in byType.entries) {
      ...
      if (entry.value.isNotEmpty && failed / entry.value.length > 0.4) {
        weak.add(entry.key);
      }
    }
    return weak;
