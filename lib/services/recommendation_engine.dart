...
class RecommendationEngine {
  ...
  LearningRecommendation generateRecommendation(
    ...
    final predictedEngagement =
        _aiEngine.predictEngagement(profile, bestMode);
    final content = _selectContent(bestMode, profile);
    final contentId = _contentIdForMode(bestMode, content);

    final reason =
        _generateReason(profile, bestMode, predictedEngagement, content);
  ...
  List<LearningRecommendation> generateMultipleRecommendations(
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!usedModes.contains(rec.recommendedMode) &&
        ...
        final remaining =
            LearningMode.values.where((m) => !usedModes.contains(m)).toList();
        if (remaining.isNotEmpty) {
          final altMode = remaining[_random.nextInt(remaining.length)];
          final content =
              _selectContent(altMode, profile, excludeIds: usedContent);
          ...
          recommendations.add(LearningRecommendation(
            ...
            estimatedDuration:
                _estimateDuration(altMode, profile.pacing, content),
  ...
  ContentItem? _selectContent(
    ...
    switch (mode) {
      ...
      case LearningMode.guidedPractice:
        candidates = ContentRepository.getByType('guided_practice');
  ...
  String _generateReason(
    ...
    final contentLine = content != null
        ? ' "${content.title}" fits your current level.'
        : '';
    if (engagement < 0.3) {
  ...
  String _generateDescription(
    ...
    final suffix = content != null ? ' Focus on "${content.title}".' : '';
